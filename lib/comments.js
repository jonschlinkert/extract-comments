...
Comments.prototype.extract = function(str) {
  ...
  var extract = opts.extractor || utils.extract;
  var ast = [];
  ...
  while (++idx < len) {
    ...
    if (isBlock(token.type) && opts.block !== false) {
    ...
    if (isLine(token.type) && opts.line !== false) {
...
function isLine(type) {
  return type === 'Line' || type === 'CommentLine';
}

function isBlock(type) {
  return type === 'Block' || type === 'CommentBlock';
}
